...
async def show_template_choice(message: Message, state: FSMContext, lang: str, edit: bool = False):
    kb = InlineKeyboardBuilder()
    kb.button(text=t("template_classic", lang), callback_data="template:classic")
    kb.button(text=t("template_modern", lang), callback_data="template:modern")
    kb.button(text=t("template_colorful", lang), callback_data="template:colorful")
    kb.adjust(1)
    if edit:
...
async def handle_admin_payment_decision(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if approved:
            ...
            kb = InlineKeyboardBuilder()
            kb.button(text=t("template_classic", user_lang), callback_data="template:classic")
            kb.button(text=t("template_modern", user_lang), callback_data="template:modern")
            kb.button(text=t("template_colorful", user_lang), callback_data="template:colorful")
            kb.adjust(1)
...
async def generate_and_send_documents(message: Message, data: dict):
    ...
    cv_template = data.get("cv_template", "classic")
